...
void StartWin() {
  ...
  if (tcgetattr(stdin->_fileno, &oldt) < 0) {
  ...
  keypad(
      scrwin,
      TRUE); // 设置把收到的特殊键比如箭头,转化为ncurses定义的以KEY开头的数字宏
...
void DrawTb(double requestTimeFloat, int concurrency, int successNum,
            ...
            char *speedStr) {
  mvwprintw(scrwin, x, y,
            DrawTd(requestTimeFloat, concurrency, successNum, failureNum, qps,
                   maxTimeFloat, minTimeFloat, avgTime, receivedBytesStr,
                   speedStr));
  refresh();
}
...
void EndWin() {
  ...
  if (tcsetattr(stdin->_fileno, TCSANOW, &oldt) < 0) {
